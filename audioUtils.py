...
def wav_to_array(fn, rescale='standardize'):
  """ Reads wav file data into a numpy array.
    ...
    Returns:
      snd, sampFreq (int, np.array): Sampling frequency of the input sound, followed
...
def play_array(snd_array, pyaudio_params={}):
  ...
  # stream = p.open(format=pyaudio.paInt16, channels=1, rate=sampFreq, output=True, frames_per_buffer=CHUNKSIZE)
