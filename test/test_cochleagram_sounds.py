...
def test_cochleagram(rfn, mode='all', verbose=0):
  ...
  try:
    for i in range(len(sample_factor_list)):
      ...
      if mode == 'all' or mode == 'nx':
        ...
        subbands = cg.generate_subband_envelopes_fast(signal,filts,None)
        print np.shape(subbands)
...
def test_run_dir(in_path, verbose=0):
  ...
  for i, f in enumerate(fntp):
    test_cochleagram(f, verbose=verbose)
